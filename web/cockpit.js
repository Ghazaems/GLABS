(() => {
  ...
  const vwapViewSignal = raw => ["BUY","HOLD"].includes(raw) ? "BULLISH" : ["REDUCE","EXIT"].includes(raw) ? "BEARISH" : "AVOID";
  const colorForVwap = raw => vwapViewSignal(raw) === "BULLISH" ? "#4ade80" : vwapViewSignal(raw) === "BEARISH" ? "#ff9f0a" : "#f87171";
  const positionLabel = p => p === "above" ? "Harga di atas" : p === "below" ? "Harga di bawah" : "Harga setara";
  const slopeLabel = n => n == null ? "—" : `${n > 0 ? "+" : ""}${Number(n).toFixed(2)}%`;
  ...
  function syntheticHistory(stock) {
    ...
    return Array.from({length: count}, (_, i) => {
      ...
      const syntheticVwap=(open+value)/2; return {date:`T-${count-i}`,open,high:Math.max(open,value)*1.008,low:Math.min(open,value)*.992,close:value,volume:1000000*(1+Math.abs(Math.sin(i*.33))),vwap:syntheticVwap,vwap_5:syntheticVwap,vwap_20:syntheticVwap,vwap_60:syntheticVwap};
    });
  }
  ...
  function makeChart(stock) {
    ...
    const values=data.flatMap(d=>[d.high,d.low,d.vwap_5??d.vwap,d.vwap_20,d.vwap_60]).filter(Number.isFinite);
    ...
    const vwapLines=[['vwap_5','#60a5fa',1.7],['vwap_20','#a78bfa',1.5],['vwap_60','#f59e0b',1.5]];
    vwapLines.forEach(([key,color,width])=>{const path=data.map((d,i)=>{const value=d[key]??(key==='vwap_5'?d.vwap:null);return Number.isFinite(value)?`${i?'L':'M'}${x(i)},${y(value)}`:''}).join(' ');if(path)svg+=`<path d="${path}" fill="none" stroke="${color}" stroke-width="${width}"/>`});
  ...
  function makeVolatilityChart(stock) {
    const v = stock.volatility;
    const hist = (v.price_history || []).slice(-90);
    const W=820,H=390, top=22,bottom=32,left=12,right=68;
    ...
    let svg=`<svg class="chart-svg" viewBox="0 0 ${W} ${H}" role="img" aria-label="Forecast volatilitas ${esc(stock.ticker)}">`;
    for(let i=0;i<5;i++){const yy=top+i*plotH/4,val=max-i*span/4;svg+=`<line x1="${left}" y1="${yy}" x2="${W-right}" y2="${yy}" stroke="#252525"/><text x="${W-right+8}" y="${yy+4}" fill="#777" font-size="10">${fmt(val)}</text>`}
    let path=''; hist.forEach((d,i)=>{path+=`${i?'L':'M'}${x(i)},${y(d.close)}`}); svg+=`<path d="${path}" fill="none" stroke="#4ade80" stroke-width="1.8"/>`;
    if(fcN){
      ...
      svg+=`<path d="${up}" fill="none" stroke="#60a5fa" stroke-width="1.3" stroke-dasharray="5 4"/><path d="${low}" fill="none" stroke="#60a5fa" stroke-width="1.3" stroke-dasharray="5 4"/><line x1="${x(startIdx)}" y1="${top}" x2="${x(startIdx)}" y2="${H-bottom}" stroke="#444" stroke-dasharray="2 3"/>`;
    }
    svg+=`<text x="${left}" y="${H-10}" fill="#666" font-size="10">${esc(hist[0]?.date||'')}</text><text x="${W-right}" y="${H-10}" fill="#666" font-size="10" text-anchor="end">+${fcN} hari ke depan</text></svg>`;
    return {svg, isReal: true, isVolatility: true};
  }

  function modePanel(stock) {
    const wy=stock.wyckoff||{}, events=wy.events||[];
    if(state.mode==='volatility'){
      const v=stock.volatility;
      if(!v||v.status!=='ok') return `<div class="panel"><h3>FORECAST VOLATILITAS</h3><p class="empty-event">Data volatilitas belum tersedia untuk saham ini (butuh minimal ~120 hari data historis).</p></div>`;
      const last=v.upper_band_95.length-1;
      return `<div class="panel"><h3>FORECAST VOLATILITAS (GARCH)</h3><div class="metric-row"><span>Regime</span><b>${esc(v.regime)}</b></div><div class="metric-row"><span>Vol. forecast harian</span><b>${v.current_forecast_vol_pct}%</b></div><div class="metric-row"><span>Vol. historis 60 hari</span><b>${v.hist_vol_60d_pct}%</b></div><div class="metric-row"><span>Band 10 hari (95%)</span><b>${fmt(v.lower_band_95[last])} – ${fmt(v.upper_band_95[last])}</b></div><p class="empty-event">Ini rentang wajar pergerakan harga berdasar pola volatilitas historis, BUKAN tebakan harga pasti dan BUKAN sinyal beli/jual. Dipakai sebagai referensi lebar stop-loss / risk sizing.</p></div>`;
    }
    ...
    if(state.mode==='vwap') {
      const a=stock.vwap_analysis||{};
      const horizons=[['Daily',a.vwap_fast],['Weekly',a.vwap_medium],['Swing',a.vwap_swing]];
      const horizonRows=horizons.map(([label,v])=>`<div class="metric-row"><span>${label} · ${v?.window||'—'}D</span><b>${fmt(v?.value)} · ${positionLabel(v?.position)} · slope ${slopeLabel(v?.slope_pct)}</b></div>`).join('');
      const reasons=(a.reasons||[]).map(reason=>`<li>${esc(reason)}</li>`).join('');
      return `<div class="panel"><h3>KEPUTUSAN VWAP MULTI-HORIZON</h3><div class="metric-row"><span>Sinyal Full VWAP</span><b style="color:${colorForVwap(stock.vwap_signal)}">${vwapViewSignal(stock.vwap_signal)}</b></div><div class="metric-row"><span>Regime</span><b>${esc(a.regime||'—')}</b></div>${horizonRows}<div class="metric-row"><span>RVOL / ATR</span><b>${fmt(a.rvol)}× / ${fmt(a.atr)}</b></div><div class="metric-row"><span>Protective stop</span><b>${fmt(a.protective_stop)}</b></div><div class="metric-row"><span>Eksekusi model</span><b>${a.execution==='next_session_open'?'Open sesi berikutnya':'Tidak ada entry baru'}</b></div>${reasons?`<ul class="checklist" style="margin-top:12px">${reasons}</ul>`:'<p class="empty-event">Data belum cukup atau belum ada alasan terkonfirmasi.</p>'}<p class="empty-event">Confidence sengaja tidak ditampilkan sebagai persentase sebelum kalibrasi out-of-sample tersedia.</p></div>`;
    }
  ...
  function render() {
    const stock=current();
    const chart=(state.mode==='volatility' && stock.volatility?.status==='ok') ? makeVolatilityChart(stock) : makeChart(stock);
    const pct=Math.max(0,Math.min(100,(stock.score+100)/2));
    document.getElementById('analysis-content').innerHTML=`<div class="drawer-shell"><div class="drawer-top"><div><div class="drawer-kicker">Decision-support cockpit · eksekusi manual</div><h2 class="drawer-title">${esc(stock.ticker)} <span style="color:var(--text-secondary)">${fmt(stock.last_close)}</span></h2><div class="drawer-sub">Skor ${stock.score} · komposit <span style="color:${colorForSignal(stock.signal)}">${esc(stock.signal.toUpperCase())}</span> · Full VWAP <span style="color:${colorForVwap(stock.vwap_signal)}">${vwapViewSignal(stock.vwap_signal)}</span> · data ${esc(state.data.generated_at?.slice(0,10)||'')}</div></div><button class="close-btn" aria-label="Tutup">×</button></div><div class="ticker-strip">${state.data.watchlist.map(x=>`<button class="ticker-chip ${x.ticker===stock.ticker?'active':''}" data-ticker="${esc(x.ticker)}">${esc(x.ticker)}</button>`).join('')}</div><div class="chart-wrap"><div class="chart-toolbar"><b>${state.mode==='volatility'?'Forecast volatilitas':'Price structure'}</b><div class="chart-legend">${state.mode==='volatility'?'<span><i class="legend-dot" style="background:#4ade80"></i>Harga historis</span><span><i class="legend-dot" style="background:#60a5fa"></i>Band 95%</span>':'<span><i class="legend-dot" style="background:#4ade80"></i>Naik</span><span><i class="legend-dot" style="background:#f87171"></i>Turun</span><span><i class="legend-dot" style="background:#60a5fa"></i>VWAP 5D</span><span><i class="legend-dot" style="background:#a78bfa"></i>VWAP 20D</span><span><i class="legend-dot" style="background:#f59e0b"></i>VWAP 60D</span>'}</div></div>${chart.svg}<div class="chart-note">${state.mode==='volatility'?'Band = rentang wajar 95%, bukan tebakan harga pasti.':(chart.isReal?'OHLC hasil screening otomatis.':'Preview estimasi; jalankan main.py agar chart memakai OHLC asli.')}</div></div><div class="mode-tabs">${[['wyckoff','Wyckoff'],['trend','Trend'],['vwap','VWAP'],['strength','Vs IHSG'],['volatility','Volatilitas'],['score','Skor']].map(([k,l])=>`<button class="mode-tab ${state.mode===k?'active':''}" data-mode="${k}">${l}</button>`).join('')}</div><div class="analysis-grid">${modePanel(stock)}<div><div class="panel"><h3>RINGKASAN MESIN</h3><div class="metric-row"><span>Skor</span><b>${stock.score}/100</b></div><div class="score-meter"><i style="width:${pct}%"></i></div><div class="decision-box">${decisionText(stock)}</div></div><div class="panel" style="margin-top:14px"><h3>CHECKLIST MANUAL SEBELUM EKSEKUSI</h3><ul class="checklist"><li>□ <span><b>Konteks pasar:</b> periksa arah IHSG dan sektor.</span></li><li>□ <span><b>Invalidasi:</b> tentukan level sebelum entry.</span></li><li>□ <span><b>Risk sizing:</b> sesuaikan jarak stop dan risiko portofolio.</span></li><li>□ <span><b>Konfirmasi:</b> tunggu price action/volume, jangan hanya skor.</span></li></ul></div></div></div></div>`;
    bindDrawer();
  }
